window.BD_MODULES = {
  gallery: { id: "gallery", name: "回忆相册", short: "照片、说明、日期", required: true },
  bgm: { id: "bgm", name: "背景音乐", short: "推荐歌库 / 填歌曲 / 上传音频", required: true },
  countdown: { id: "countdown", name: "生日倒计时", short: "由生日自动生成", required: true },
  messageWall: { id: "messageWall", name: "留言墙", short: "留言、署名、可选照片", required: false },
  wishBottle: { id: "wishBottle", name: "许愿瓶", short: "标题、提示语、愿望数量、隐私", required: false },
  surpriseBox: { id: "surpriseBox", name: "好友盲盒", short: "标题、内容、开启时间", required: false },
  playlist: { id: "playlist", name: "生日歌单", short: "最多 10 首歌和推荐理由", required: false },
  partyChecklist: { id: "partyChecklist", name: "派对清单", short: "3–10 个生日任务", required: false },
  hiddenEgg: { id: "hiddenEgg", name: "隐藏彩蛋", short: "触发方式、标题、内容", required: false },
  birthdayMap: { id: "birthdayMap", name: "生日地图", short: "地点、回忆、照片、地图链接", required: false },
  futureMailbox: { id: "futureMailbox", name: "未来信箱", short: "正文与开启日期", required: false },
  giftVote: { id: "giftVote", name: "礼物投票", short: "2–6 个礼物选项", required: false },
  dailyLuck: { id: "dailyLuck", name: "今日好运", short: "是否开启与出生时间", required: false },
  multiContributor: { id: "multiContributor", name: "好友共创", short: "征集链接、审核状态", required: false }
};
